...
int main(void)
{
  ...
  osMessageQDef(buttonQueue, 2, uint8_t);
  buttonQueueHandle = osMessageCreate(osMessageQ(buttonQueue), NULL);

  /* definition and creation of randomQueue */
  osMessageQDef(randomQueue, 2, uint8_t);
  ...
  /* definition and creation of randomTask */
  osThreadDef(randomTask, StartRandomTask, osPriorityNormal, 0, 128);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
  if (GPIO_Pin == GPIO_PIN_13) {
    uint8_t pressed = (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET);
    uint8_t msg = pressed ? BUTTON_SRC_EXTI_PRESSED : BUTTON_SRC_EXTI_RELEASED;
    if (buttonQueueHandle != NULL) {
      osMessagePut(buttonQueueHandle, msg, 0);
    }
  }
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  else
  {
	  if(htim->Instance == TIM3) {
		  uint8_t pressed = (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET);
		  uint8_t msg = pressed ? BUTTON_SRC_TIM_PRESSED : BUTTON_SRC_TIM_RELEASED;
		  if (buttonQueueHandle != NULL) {
			  osMessagePut(buttonQueueHandle, msg, 0);
		  }
		}
